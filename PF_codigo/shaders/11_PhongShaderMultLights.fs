...
{    
    ...
    // Si la intensidad supera los límites físicos, se procesa como luz pura.
    if (MaterialAmbientColor.r > 1.0 || MaterialAmbientColor.g > 1.0 || MaterialAmbientColor.b > 1.0) {
        // Se renderiza únicamente el color del láser y su nivel de transparencia actual
        FragColor = vec4(MaterialAmbientColor.rgb, transparency);
    } else {
        // Comportamiento de renderizado normal para el resto de los objetos
